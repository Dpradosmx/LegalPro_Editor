    ...
    m_debug(debug)
{
    ...
    opciones<<"mensaje"<<"syncronizacionResultado"<<"loginRespuesta"<<"respuestainstalacion"<<"respuestaconfirmacion";
    //0,1,2....,n
}
...
void websocket_syncronizacion::insertar(){
    ...
    myTimer.start();
      ...
      QSqlQuery query(db);
      QString script="delete from till where id_opr !=100";
      QStringList scriptQueries = script.split(";");
      foreach (QString queryTxt, scriptQueries)
          {
              if (queryTxt.trimmed().isEmpty()) {
...
void websocket_syncronizacion::select(){
    ...
    QSqlQuery query(db);
    query.exec("select * from till");
    while (query.next()) {

        qDebug() << "id" << query.value("id_opr").toString()<<query.value("id_rpsty_tnd").toString();
    }
...
void websocket_syncronizacion::onTextMessageReceived(QString message)
{
    ...
    case 1:{
        ...
        break;
    case 2:
        qDebug()<< "respuesta login" <<obj["value"].toString().split(",").at(1);
        emit loginf(91,1);
        break;
    case 3:
